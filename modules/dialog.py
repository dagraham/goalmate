...
menu_keys = ["f1", "f2", "f3", "f4", "f5", "f6", "f7", "f8", "c-q", "l", "n"]
...
for key in select_keys:
    kb.add(key, filter=Condition(lambda: dialog.mode == "select"), eager=True)(
        lambda event, key=key: select_tag(event, key)
    )
...
for key in bool_keys:
    kb.add(key, filter=Condition(lambda: dialog.mode == "boolian"), eager=True)(
        lambda event, key=key: select_boolian(event, key)
    )
